...
function dropdown() {
    ...
    mountainsArray.forEach(loc => {
        ...
        option.textContent = loc.name; // Use 'name' instead of 'Name'
    ...
    select.addEventListener("change", () => {
        const selectedLocationName = select.value;
        const selectedLocation = mountainsArray.find(loc => loc.name === selectedLocationName); // Use 'name' instead of 'LocationID'
        if (selectedLocation) {
            const displayDiv = document.getElementById("results");

            // Log the image path to the console for debugging
            console.log("Image path:", selectedLocation.img);

            displayDiv.innerHTML = `
            <div class="location-detail"><span>Name:</span> ${selectedLocation.name}</div>
            <div class="location-detail"><span>Elevation:</span> ${selectedLocation.elevation} feet</div>
            <div class="location-detail"><span>Effort:</span> ${selectedLocation.effort}</div>
